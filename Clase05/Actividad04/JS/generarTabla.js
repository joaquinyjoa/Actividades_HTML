...
function mostrarTabla(remerasSinJson = []) {
    ...
    const titulos = ["Id", "Slogan", "Size", "Price", "Color", "Name", "Logo", "Country", "City", "ACCION"];
    ...
    remerasSinJson.forEach(r => {
        ...
        // Celda con enlace "Eliminar"
        let celdaAccion = document.createElement("td");
        let linkEliminar = document.createElement("a");
        linkEliminar.href = "#"; // Para que parezca un enlace
        linkEliminar.textContent = "Eliminar";

        linkEliminar.addEventListener("click", (e) => {
            e.preventDefault(); // Evita que navegue
            // Confirmar eliminación
            if (confirm(`¿Seguro que querés eliminar la remera con ID ${r.id}?`)) {
             // Buscar el índice del objeto por ID
            const indexAEliminar = remerasSinJson.findIndex(remera => remera.id === r.id);

            if (indexAEliminar !== -1) {
                // Eliminar del array
                remerasSinJson.splice(indexAEliminar, 1);

                // Actualizar el localStorage
                localStorage.setItem("remeras", JSON.stringify(remerasSinJson));
        ...
        celdaAccion.appendChild(linkEliminar);
